...
{
    NSArray * arr = [[NSArray alloc]initWithObjects:@"dinosaur", @"catfish", @"spoon", @123, @[], nil];
    return arr;
}
             ...
             withArrayTwo:(NSArray *)arrayTwo
{
    ...
    NSMutableArray *joined = [arrayOne mutableCopy];
    [joined addObjectsFromArray: arrayTwo];
    return joined;
}
...
- (NSString *)returnTheLastStandardResolutionImageURL:(NSDictionary *)JSON
{
    ...
    NSString *url = [[[lastPost objectForKey:@"images"] objectForKey:@"standard_resolution"] objectForKey:@"url"];
    return url;
}
